...
![SEPA XML Viewer (Windows, dark theme) showing one of 2500 payroll transactions selected in the 2500-tx stress fixture](docs/screenshots/02-viewer-loaded.png)

The above is the viewer on Windows (dark theme follows the OS) with the 2500-transaction stress fixture loaded: tree of group header → payment-info batch → individual transactions on the left, fields of the selected transaction on the right, with a Raw XML tab one click away. The status bar at the bottom summarises the file ("2500 transactions across 1 batch · 2.7 MB").
